...
print "    'domainname' => '%(domainname)s'," % configRegistry
@!@
     ...
     */
@!@print "    'debug' => %s," % ('true' if configRegistry.is_true('saml/idp/log/debug/enabled') else 'false',)@!@
...
@!@print "    'logging.level' => SimpleSAML_Logger::%s," % (configRegistry.get('saml/idp/log/level', 'ERR'),)@!@
     ...
     */
@!@print "    'enable.saml20-idp' => %s," % ('true' if configRegistry.is_true('saml/idp/enableSAML20-IdP') else 'false',)@!@
...
for key, server in configRegistry.iteritems():
