		...
		try:
			...
			for result in self.s4_search_ext_s(compatible_modstring(object['dn']), ldap.SCOPE_SUBTREE, 'objectClass=*'):
				...
				if not self._ignore_object(key, back_mapped_subobject):
					if not self.sync_from_ucs(key, subobject_s4, back_mapped_subobject['dn']):  # FIXME: this call is wrong: sync_from_ucs() must be called with a ucs_object not with a samba_object!
						ud.debug(ud.LDAP, ud.WARN, "delete of subobject failed: %r" % (result[0],))
						return False

			# FIXME: endless recursion if there is one subtree-object which is ignored or can't be removed.
