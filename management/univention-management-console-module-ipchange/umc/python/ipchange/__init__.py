...
class Instance(Base):
	...
	def change(self, role, ip, netmask, oldip=None):
		...
		network = ipaddress.IPv4Network(u'%s/%s' % (ip, netmask), strict=False)
		...
		if oldip and oldip != ip:
			...
			for forwardobject in univention.admin.modules.lookup(fmodule, None, lo, scope='sub', filter=filter_format('(aRecord=%s)', (oldip,))):
				forwardobject.open()
		...
		for e in list(server['dnsEntryZoneReverse']):
