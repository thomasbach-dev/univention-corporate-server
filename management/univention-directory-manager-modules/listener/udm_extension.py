...
def handler(dn, new, old):
	...
	if new and not old:
		if listener.configRegistry.get('server/role') == 'domaincontroller_master':
			...
			cmp_start_vs_50 = apt.apt_pkg.version_compare(univentionUCSVersionStart, "5.0")
			# cmp_end_vs_499 = apt.apt_pkg.version_compare(univentionUCSVersionEnd, "4.99")
			## Keep object if cmp_start_vs_50 != -1 [i,e, Py3] or (cmp_start_vs_50 == -1 and univentionUCSVersionEnd) [or cmp_end_vs_499 == 0]
			# Otherwise remove it:
			if cmp_start_vs_50 == -1 and not univentionUCSVersionEnd:
				ud.debug(ud.LISTENER, ud.INFO, '%s: Removing incompatible extension %s.' % (name, new['cn'][0].decode('UTF-8')))
