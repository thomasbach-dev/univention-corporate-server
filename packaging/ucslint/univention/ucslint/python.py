...
def _or(*disjunct: str, name: str = None) -> str:
    return r"(?%s%s)" % (":" if name is None else f"P<{name}>", "|".join(disjunct))


RE_BANG = re.compile(r"^#!.*[ /]python[0-9.]*\b")
ESCAPE_LENIENT = r"\\."
ESCAPE_RAW = r"\\(?:$|.)"
...
def python_files(path: str) -> Iterator[str]:
    ...
    for path in FilteredDirWalkGenerator(path, suffixes=SUFFIXES):
        yield path

    for path in FilteredDirWalkGenerator(path, ignore_suffixes=SUFFIXES, reHashBang=RE_BANG):
        yield path
